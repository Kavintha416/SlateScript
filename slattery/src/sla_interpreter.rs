...
impl UiInterpreter {
    ...
    fn parse_property_into_component(
        ...
        let prop_name: String = match tokens.get(i) {
            Some(UiToken::Identifier(name)) => {
                // Check if this is actually an event handler
                if name == "on_click" || name == "on_tap" || name == "on_change" || name == "on_input" {
                    name.clone()
                } else {
                    name.clone()
                }
    ...
    fn parse_component_instantiation(&mut self, tokens: &[UiToken], start: usize) -> Result<Option<(Rc<RefCell<UiComponent>>, usize)>, String> {
        ...
        // Find the next UI component type
        let component_type = loop {
            if i >= tokens.len() {
                return Err("Unexpected end of tokens".to_string());
            }
            
            let component = match &tokens[i] {
                UiToken::Window => Some("Window".to_string()),
                UiToken::Column => Some("Column".to_string()),
                UiToken::Row => Some("Row".to_string()),
                UiToken::Text => Some("Text".to_string()),
                UiToken::Button => Some("Button".to_string()),
                UiToken::Input => Some("Input".to_string()),
                UiToken::Parent | UiToken::Child => {
                    return Err("Parent/Child relationships should be inside component braces".to_string());
                }
                // Skip SlateScript keywords and comments
                UiToken::Make | UiToken::Func | UiToken::Import | UiToken::From | 
                UiToken::Identifier(_) | UiToken::String(_) | UiToken::Number(_) | 
                UiToken::Boolean(_) | UiToken::True | UiToken::False |
                UiToken::LeftParen | UiToken::RightParen | UiToken::Semicolon |
                UiToken::LessThan | UiToken::GreaterThan | UiToken::Equal | UiToken::Slash => None,
                _ => return Err(format!("Expected component type, got: {:?}", tokens[i])),
            };
        ...
        if i < tokens.len() && matches!(tokens[i], UiToken::LeftBrace) {
            ...
            while i < tokens.len() && !matches!(tokens[i], UiToken::RightBrace) {
                match &tokens[i] {
                    ...
                    _ => {
                        // This is where properties are parsed
    ...
    fn parse_simple_component_reference(&mut self, tokens: &[UiToken], start: usize) -> Result<Option<(Rc<RefCell<UiComponent>>, usize)>, String> {
        ...
        // Find the next UI component type, skipping SlateScript keywords
        let component_type = loop {
            if i >= tokens.len() {
                return Err("Unexpected end of tokens".to_string());
            }
            ...
            let component = match &tokens[i] {
                ...
                UiToken::LeftParen | UiToken::RightParen | UiToken::Semicolon |
                UiToken::LessThan | UiToken::GreaterThan | UiToken::Equal | UiToken::Slash => None,
                UiToken::LeftBrace | UiToken::RightBrace => {
                    // Found a brace while looking for component type - this means
                    // we're not in a component reference, return None
                    return Ok(None);
                }
                _ => return Err(format!("Expected component type, got: {:?}", tokens[i])),
            };
