...
impl StyleEngine {
    ...
    // Parse style blocks from .st file content
    pub fn parse_style_blocks(&mut self, source: &str) -> Result<(), String> {
        let mut interpreter = StyleInterpreter::new();
        let rules = interpreter.parse_style_blocks(source)?;
    ...
    pub fn load_from_file(&mut self, file_path: &str) -> Result<(), String> {
        ...
        self.parse_style_blocks(&content)
    }
}
