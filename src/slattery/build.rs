...
pub fn build_app(config: &BuildConfig) -> Result<(), String> {
    ...
    let styles_src = config.app_root.join("styles");
    let styles_dest = build_dir.join("styles");
    if styles_src.exists() {
        fs::create_dir_all(&styles_dest)
            .map_err(|e| format!("Failed to create styles dir: {}", e))?;
        copy_directory(&styles_src, &styles_dest, "sts")?;
        println!("  [OK] Copied styles");
    }
...
pub fn find_slate_dir() -> Result<PathBuf, String> {
    ...
    let common_paths = vec![
        "C:\\Users\\Kavintha Piyasekara\\Desktop\\SlateScript-main".to_string(),
...
fn copy_executable(build_dir: &Path, output_dir: &Path, app_name: &str, release: bool) -> Result<(), String> {
    ...
    // Copy styles/ next to the exe (optional — only if present)
    let styles_src = abs_build_dir.join("styles");
    if styles_src.exists() {
        let styles_dest = output_dir.join("styles");
        fs::create_dir_all(&styles_dest)
            .map_err(|e| format!("Failed to create output styles dir: {}", e))?;
        copy_directory(&styles_src, &styles_dest, "sts")?;
        println!("[OK] Copied styles/ next to executable");
    }
